...
class Fanv2Accessory extends BaseAccessory {
  ...
  refreshAccessoryServiceIfNeed(statusArr, isRefresh) {
    ...
    for (const statusMap of statusArr) {
      ...
      if (statusMap.code === 'fan_speed') {
        this.speedMap = statusMap;
        this.log.log("speedMap: %s", JSON.stringify(this.speedMap))
    ...
    switch (name) {
      ...
      case Characteristic.RotationSpeed:
        ...
        if ((typeof this.speedMap.value == 'string') && this.speedMap.value.constructor == String) {
          ...
          speed = Math.floor((this.speed_range.max - this.speed_range.min) * hbValue / 100)
          this.log.log("Mapped speed from hbValue: %s to value: %s. min: %s max: %s", hbValue, speed, this.speed_range.min, this.speed_range.max);
        }
  ...
  getSpeedFunctionRange(code) {
    ...
    if (funcDic) {
      this.log.log("funcDic: %s", JSON.stringify(funcDic))
  ...
  getSpeedFunctionLevel(code) {
    ...
    var funcDic = this.functionArr.find((item, index) => { return item.code == code })
    this.log.log("getSpeedFunctionLevel funcDic: %s", JSON.stringify(funcDic))
    if (funcDic) {
