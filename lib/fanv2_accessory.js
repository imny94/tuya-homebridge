  ...
  getSendParam(name, hbValue) {
    ...
    switch (name) {
      ...
      case Characteristic.ColorTemperature:
        ...
        value = Math.round(homekitColourTempMax - ((hbValue - homekitColourTempMin) / (currMax - currMin)) * (homekitColourTempMax - homekitColourTempMin));
        this.log.log("[%s] Mapped ColorTemperature from hbValue: %s to value: %s. min: %s max: %s", this.deviceConfig.name, hbValue, value, currMin, currMax);
        break;
      default:
        break;
    }
